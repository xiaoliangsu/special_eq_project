  ...
  <div class="AcceptRegist">
    ...
    <div class="body">
      ...
      <RadioGroup v-model="choose" @on-change="chooseRole" style="margin-left:100px;margin-bottom:10px;">
          ...
          <FormItem prop="username" label="用户名">
            <Poptip trigger="focus" placement="right">
              <div slot="content" style="white-space: normal;">
                <p>至少5个字符，不允许中文，只允许英文和数字，不要求和机关名字一致</p>
              </div>
              <Input v-model="registInfo.username" placeholder="请设置用户名" style="ime-mode:disabled"></Input>
            </Poptip>
        ...
        <Row>
          <FormItem prop="name" label="登记人员" v-if="this.approver">

            <Input v-model="registInfo.name" placeholder="填写负责登记工作的人员姓名"></Input>
        ...
        <Row>
          <FormItem prop="verifyId" label="身份证号" v-if="this.approver">

            <Input v-model="registInfo.verifyId" placeholder="请填入登记人员的公民身份证号码"></Input>
  ...
  export default {
    data() {
      ...
      const nameConfirm = (rule, val, callback) => {
        if (this.registInfo.password !== this.registInfo.password2) {
          callback(new Error('两次密码填写不相同'));
        }else if (this.registInfo.username.length < 5) {
          callback(new Error('请输入至少5个字符'));
        }else{
          callback();
        }
