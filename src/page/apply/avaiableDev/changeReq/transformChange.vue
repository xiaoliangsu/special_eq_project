...
<!--             <FormItem label="登记机关" class="ivu-form-item-required">
        ...
        <div class="pdfInfo" v-if="this.active==4">
          ...
          <Row style="width:1000px;">
            ...
            <Col span="10">
            <Form-item label="重新填写的使用登记表" :label-width="200">
              <Upload
                ref="upload2"
                :format="['pdf']"
                :on-format-error="handleFormatError"
                ...
                :on-remove="handleRemove"
                :default-file-list="defaultPdfList1"
                :action="'/file/upload?applyId='+this.applyId+'&fileName='+'重新填写的使用登记表'"
                with-credentials>
                <Button type="ghost" icon="ios-cloud-upload-outline">上传文件</Button>
            ...
            <Form-item label="改造质量证明资料" :label-width="200">
              <Upload
                ...
                :on-remove="handleRemove"
                :default-file-list="defaultPdfList1"
                :action="'/file/upload?applyId='+this.applyId+'&fileName='+'改造质量证明资料'"
                with-credentials>
                <Button type="ghost" icon="ios-cloud-upload-outline">上传文件</Button>
              </Upload>
            </Form-item>
            ...
            <Form-item label="改造监督检验证书" :label-width="200">
              <Upload
                ...
                :on-remove="handleRemove"
                :default-file-list="defaultPdfList1"
                :action="'/file/upload?applyId='+this.applyId+'&fileName='+'改造监督检验证书'"
                with-credentials>
                <Button type="ghost" icon="ios-cloud-upload-outline">上传文件</Button>
              </Upload>
            </Form-item>
      ...
      instance (type) {
        ...
        setAppService.confrimApp(params).then(res => {
          if (res) {
            this.$Modal.confirm({
              title: '通知',
              content: '<p>您已经成功提交变更申请</p><p>请耐心等待受理结果</p>',
              onOk: () => {
                this.$router.go(0);
              },
              onCancel: () => {
